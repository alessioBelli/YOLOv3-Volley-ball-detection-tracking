        ...
        if self.initBB is None:            
            ...
            for i in range(len(self.points)):
                if i != (len(self.points))-1:
                    cv2.line(frame, self.points[i], self.points[i+1],(255,0,100), 2, 1)
            for i, bbox in enumerate(boxes):
                if scores[0]>0.40:              #inizializzo il tracker se la confidence della detection è sufficientemente alta
                    ...
                    self.coordinata_x = boxes[0][0]
                    self.coordinata_y = boxes[0][1]
                    self.points.append((int(self.coordinata_x), int(self.coordinata_y)))
...
def opencv_tracking(video_path, detection_path, out_txt_file):
    ...
    print ("elapsed: "+str(elapsed)+ " total_frame: "+str(frame_id)+" fps : "+str(fps))
